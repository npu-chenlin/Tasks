...
void MainWindow::mouseReleaseEvent(QMouseEvent *event)
{
    int x0 = event->x() - 40;
    int y0 = event->y() - 40;
    if (!(x0 >= 0 && x0 <= 40 * (SIZE - 1) && y0 >= 0 && y0 <= 40 * (SIZE - 1))) return;
    int lx = x0 / 40;
    int ly = y0 / 40;
    bool flag = false;
    for (int i = lx; i < lx + 2 && !flag; ++i)
    {
        ...
        {
            if (std::sqrt(std::pow((x0 - (20 + i * WIDTH)), 2) + std::pow((y0 - (20 + j * WIDTH)), 2)) < 20)
            {
                if (m[i][j]) return;
                flag = true;
                ++steps;
...
void MainWindow::dropChessman(bool player)
{
    int attackScore = 0;
    int defenceScore = 0;
    int value = 0;
    ...
    qDebug() << m0 << " " << n0 << " " << attackScore << ";" << m1 << " " << n1 << " " << defenceScore;
    if (player)
    {
...
{
    //连5，活四，死四，活三，死三，活二，死二
    int type[] = {400000, 100000, 3000, 3000, 1500, 700, 200, 0};
    int value = 0;
    int forword = 0;
    int post = 0;
    bool empty = false;
